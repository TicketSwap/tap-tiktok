...
class AdAccountsStream(TikTokStream):
    ...
    def get_url_params(self, context: Optional[dict], next_page_token: Optional[Any]) -> Dict[str, Any]:
        advertiser_id = self._resolve_advertiser_id(context)
        params: dict = {"advertiser_ids": json.dumps([str(advertiser_id)])}
        if next_page_token:
        ...
        if isinstance(next_page_token, dict) and next_page_token["start_date"] is not None:
            ...
            if start_date is None:
                if not self.config.get("start_date"):
                    raise RuntimeError("Missing start_date. Set `start_date` in config.")
    ...
    def request_records(self, context: Optional[dict]) -> Iterable[dict]:
        ...
        while not finished:
            ...
            if next_page_token and next_page_token == previous_token:
                raise RuntimeError(
                    f"Loop detected in pagination. Pagination token {next_page_token} is identical to prior token."
                )
